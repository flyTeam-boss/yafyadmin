<template>
    <div class="app-container">
        ...
        <div class="formBox">
                ...
                <div class="dialogBox">
                    <el-form-item  label="考试时长：" label-width="95px" prop="limitTime">
                        <el-input type="number" v-model.number="form.limitTime" placeholder="输入考试时长（分钟）" />
                    </el-form-item>
                    <el-form-item label="公开答案：" label-width="95px" prop="transparent">
                        <el-select v-model="form.transparent">
                            <el-option
                            v-for="item in transparentList"
                            :key="item.id"
                            :label="item.name"
                            :value="item.id">
                            </el-option>
...
export default {
    ...
    data(){
        return {
            ...
            transparentList:[{id:0,name:'公开（学生自主做完后）'},{id:1,name:'考试结束后（截至时间）'},{id:2,name:'非公开'}],
            listQuery: {
                page: 1,
                limit: 20,
                subjectId: '',
            ...
            rules: {
                ...
                limitTime: [{ required: true, message: "请输入考试时长", trigger: "change" }],
                transparent: [{ required: true, message: "请选择是否公开答案", trigger: "change" }],
    ...
    created(){
        ...
        this.$API.specialPractice.limitTimePracticeEdit(this.$route.query.id).then((res)=>{
            this.form = {
                ...res.data.object,
                questionIds:[]
            }
            this.studyList = res.data.object.questionBankResponses
    ...
    methods:{
        saveClass(){
            this.$refs['dataForm'].validate(valid => {
                ...
                this.$API.specialPractice.limitTimePracticeSave({
                    ...this.form
                }).then(Response => {
                    ...
                    this.restForm()
                    this.$router.push({path:'simulationindex'})
